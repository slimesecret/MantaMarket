...
// ── Lấy sellerId từ session nếu chưa được truyền từ file cha ──
if (!isset($sellerId) || !$sellerId) {
    session_start(); // Bỏ dòng này nếu session đã được start ở bootstrap.php
    $sellerId = intval($_SESSION['seller_id'] ?? 0);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete' && $id) {
    ...
    header("Location: /MantaMarket/sellers/index.php#products");
    exit();
}

// ── XÓA NHIỀU SẢN PHẨM (chỉ của seller hiện tại) ──
...
// ── THÊM SẢN PHẨM MỚI (seller_id luôn lấy từ session) ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'insert') {
    // seller_id bắt buộc lấy từ session, không nhận từ POST
    ...
    if (!$sellerId || !$name || !$slug) {
        if ($isAjax) {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Vui lòng điền đủ tên và slug.']);
            exit();
        }
        header("Location: /MantaMarket/sellers/index.php#products");
        exit();
    ...
    $newId = $products->buildQueryParams([
        ...
        "value" => [
            ":seller_id"   => $sellerId,   // luôn dùng $sellerId từ session
            ":brand_id"    => $brand_id,
            ":name"        => $name,
            ":slug"        => $slug,
...
// ── LẤY DỮ LIỆU DANH SÁCH (chỉ của seller hiện tại) ──
$productsList = $products->buildQueryParams([
    "where"  => "seller_id = :sid",
    "params" => [":sid" => $sellerId]
])->select();
...
<div class="modal-bg" id="addModal">
    <div class="modal-box" style="max-width:560px;max-height:90vh;overflow-y:auto">
        ...
        <div style="display:grid;gap:14px">
            ...
            <!-- KHÔNG có trường Seller — tự động gán từ session -->
            <div>
...
<script>
    ...
    // Seller hiện tại (từ PHP session) — không cần sellers array nữa
    const currentSellerId = <?= (int)$sellerId ?>;
    ...
    (function initSelects() {
        ...
        brands.forEach(b => {
            selBrand.appendChild(new Option(b.name, b.id));
            filterBrand.appendChild(new Option(b.name, b.id));
        });
    })();

    // ═══════════════════════ HẰNG SỐ ═══════════════════════
    const STATUS_LABEL = {
        active:   { text: 'Hiển thị',  cls: 'badge-active'   },
        inactive: { text: 'Đang ẩn',   cls: 'badge-inactive'  },
        draft:    { text: 'Bản nháp',  cls: 'badge-draft'     },
        banned:   { text: 'Bị khoá',   cls: 'badge-banned'    },
    };
    const PAGE_SIZE = 8;
    let currentPage = 1;
    ...
    function renderProducts() {
        ...
        document.getElementById('productTableBody').innerHTML = slice.map(p => {
            ...
            const stl = STATUS_LABEL[p.status] || { text: p.status, cls: '' };
            return `<tr>
    ...
    function deleteProduct(i) {
        ...
        const params = new URLSearchParams({ ajax: '1', action: 'delete', id: p.id });
        fetch('/MantaMarket/sellers/index.php?page=products', { method: 'POST', body: params })
            .then(r => r.json())
            .then(res => {
    ...
    function confirmBulkDelete() {
        ...
        const params = new URLSearchParams({ ajax: '1', action: 'bulk_delete' });
    ...
    function insertProduct() {
        ...
        // seller_id KHÔNG gửi lên — server tự lấy từ session ($sellerId)
    ...
    function viewProduct(productArrIdx) {
        ...
        fetch(`/MantaMarket/sellers/products/ajax_view.php?id=${p.id}`)
            .then(r => r.json())
            .then(data => {
                ...
                const BADGE = v => {
                    const st = parseInt(v.stock_quantity), al = parseInt(v.low_stock_alert || 5);
                ...
                const statusOpts  = [['draft','Bản nháp'],['active','Hiển thị'],['inactive','Ẩn'],['banned','Bị khoá']]
                ...
                <div style="background:#faf5ff;border:1.5px dashed #c4b5fd;border-radius:10px;padding:16px;margin-top:4px">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-bottom:10px">
                        ${['sku','color','size','material','price','compare_price','stock_quantity','low_stock_alert'].map(f => {
                            const labels = {sku:'SKU',color:'Màu sắc',size:'Size',material:'Chất liệu',price:'Giá bán (VNĐ) *',compare_price:'Giá so sánh',stock_quantity:'Tồn kho',low_stock_alert:'Cảnh báo SH'};
                            const placeholders = {sku:'SKU-001',color:'Đen, Trắng...',size:'S, M, L...',material:'Cotton...',price:'0',compare_price:'0',stock_quantity:'0',low_stock_alert:'5'};
                            const isNum = ['price','compare_price','stock_quantity','low_stock_alert'].includes(f);
                            return `<div>
                                <div style="font-size:11px;color:#999;margin-bottom:4px">${labels[f]}</div>
                                <input id="nv-${f.replace(/_/g,'-')}" ${isNum ? 'type="number" min="0" step="'+(f==='price'||f==='compare_price'?'1000':'1')+'"':''} placeholder="${placeholders[f]}"
                ...
                const imagesBaseHtml = images.length ? `
                ...
                const imagesEditHtml = imagesBaseHtml + `
                <div style="margin-top:14px;background:#f0f9ff;border:1.5px dashed #7dd3fc;
                    ...
                    <div style="display:flex;gap:10px;align-items:flex-start;flex-wrap:wrap">
                        <div style="flex:1;min-width:200px">
                            ...
                            <div style="font-size:11px;color:#94a3b8;margin-top:4px">JPG, PNG, WEBP — tối đa 5MB/ảnh</div>
                        </div>
                ...
                <div style="background:#fff;border:1px solid #f0f0f0;border-radius:8px;overflow:hidden">
                    ...
                    <div style="padding:18px">
                        ...
                        <div class="vp-panel"        id="vp-edit-images">${imagesEditHtml}</div>
                    </div>
                </div>`;
    ...
    function vpSelectColor(name, el) {
        document.querySelectorAll('#vpColorChips .vp-color-chip').forEach(c => {
            ...
            c.style.color  = '#333';
            c.querySelectorAll('span').forEach(s => { if (s.style.position === 'absolute') s.remove(); });
        });
        ...
        if (imgUrl) { const img = document.getElementById('vpMainImg'); if (img) img.src = imgUrl; }
        vpUpdatePrice();
    ...
    function vpSelectSize(sz, el) {
        ...
        document.querySelectorAll('#vpSizeChips .vp-size-chip').forEach(c => {
            c.classList.remove('active'); c.style.border = '1px solid #e0e0e0'; c.style.color = '#333';
        });
        el.classList.add('active'); el.style.border = '1.5px solid #ee4d2d'; el.style.color = '#ee4d2d';
    ...
    function vpUpdatePrice() {
        ...
        if (!v) return;
        const price = parseFloat(v.price), cmp = parseFloat(v.compare_price || 0);
        document.getElementById('vpPriceMain').textContent = window._vpFmtVND(price);
        const cmpEl = document.getElementById('vpPriceCmp'), pctEl = document.getElementById('vpPricePct');
        if (cmp > price) {
            cmpEl.textContent = window._vpFmtVND(cmp); cmpEl.style.display = 'inline';
            pctEl.textContent = '-' + Math.round((1 - price / cmp) * 100) + '%'; pctEl.style.display = 'inline';
        } else { cmpEl.style.display = pctEl.style.display = 'none'; }
        const st = parseInt(v.stock_quantity), al = parseInt(v.low_stock_alert || 5);
    ...
    function _vpPost(params) {
        return fetch('/MantaMarket/sellers/products/ajax_edit.php', {
    ...
    function _vpToInput(el, type = 'text', extra = '') {
        ...
        if (type === 'textarea') {
            ...
            setTimeout(() => { const s = el.querySelector('[data-inline-input]'); if (s) s.value = cur; }, 0);
    ...
    function vpEditField(id, type)  { const el = document.getElementById(id); if (!el || el.querySelector('[data-inline-input]')) return; _vpToInput(el, type, type === 'number' ? 'width:140px' : ''); }
    function vpEditSelect(id)       { const el = document.getElementById(id); if (!el || el.querySelector('[data-inline-input]')) return; _vpToInput(el, 'select', el.dataset.opts || ''); }
    ...
    function vpSaveBasic(productId, productArrIdx) {
        ...
        // seller_id không cần gửi — server giữ nguyên (hoặc lấy từ session)
        _vpPost({ section:'basic', product_id:productId, name, slug, description:desc,
                  base_price:String(price).replace(/[^0-9.]/g,''), brand_id:brand,
                  seller_id:currentSellerId, status, is_featured:featured })
            .then(res => {
                if (res.status !== 'success') return _vpMsg('info-msg', false, res.message || 'Lỗi');
                const p = products[productArrIdx];
                p.name = name; p.slug = slug; p.description = desc;
    ...
    function vpSaveVariants(productId) {
        ...
        _vpPost({ section:'variants', product_id:productId, variants_json:JSON.stringify(variants) })
            .then(res => {
    ...
    function vpSaveAttrs(productId) {
        ...
        _vpPost({ section:'attributes', product_id:productId, attributes_json:JSON.stringify(attrs) })
            .then(res => {
    ...
    function vpAddTag() {
        ...
        if (!container) return;
        const exists = [...container.querySelectorAll('.et-tag')].some(el => el.dataset.tag === val);
        if (exists) { inp.value = ''; inp.focus(); return; }
    ...
    function vpSaveTags(productId) {
        const inp = document.getElementById('et-input');
        if (inp && inp.value.trim() !== '') vpAddTag();
        const container = document.getElementById('et-tags');
        const tags = container ? [...new Set(
            [...container.querySelectorAll('.et-tag')]
            .map(el => (el.dataset.tag ?? '').trim()).filter(t => t !== '')
        )] : [];
        if (!productId) return _vpMsg('et-msg', false, 'Thiếu product_id');
        _vpMsg('et-msg', true, 'Đang lưu...');
        _vpPost({ section:'tags', product_id:productId, tags_json:JSON.stringify(tags) })
            .then(res => {
                if (res.status !== 'success') return _vpMsg('et-msg', false, res.message || 'Lỗi khi lưu');
    ...
    function vpSetPrimary(productId, imageId) {
        _vpPost({ section:'image_set_primary', product_id:productId, image_id:imageId })
            .then(res => {
    ...
    function vpAddVariant(productId, productArrIdx) {
        ...
        if (!price || Number(price) <= 0) return _vpMsg('nv-msg', false, 'Vui lòng nhập giá bán');
        const payload = {
            section: 'variant_add', product_id: productId,
            ...
            price,
            compare_price:  document.getElementById('nv-compare-price').value || '0',
            stock_quantity: document.getElementById('nv-stock-quantity').value || '0',
            low_stock_alert:document.getElementById('nv-low-stock-alert').value || '5',
            is_active: '1',
        };
    ...
    function vpDeleteImage(productId, imageId, btn) {
        ...
        _vpPost({ section:'image_delete', product_id:productId, image_id:imageId })
            .then(res => {
                if (res.status !== 'success') return alert('Lỗi: ' + (res.message || ''));
    ...
    // ═══════════════════════ EVENT LISTENERS ═══════════════════════
    document.getElementById('productSearch').addEventListener('input',  () => { currentPage = 1; renderProducts(); });
    document.getElementById('statusFilter').addEventListener('change',  () => { currentPage = 1; renderProducts(); });
    document.getElementById('brandFilter').addEventListener('change',   () => { currentPage = 1; renderProducts(); });
    document.getElementById('addModal').addEventListener('click',  e => { if (e.target.id === 'addModal')  closeAddModal(); });
    document.getElementById('viewModal').addEventListener('click', e => { if (e.target.id === 'viewModal') e.target.style.display = 'none'; });
    ...
    function vpUploadImages(productId, productArrIdx) {
        ...
        if (!files || files.length === 0) return _vpMsg('img-upload-progress', false, 'Vui lòng chọn ít nhất 1 ảnh');
        ...
        const uploadOne = (file) => {
            if (file.size > 5 * 1024 * 1024) { failed++; return Promise.resolve(); }
            const fd = new FormData();
            fd.append('section',    'image_upload');
            fd.append('product_id', productId);
            fd.append('color',      color);
            fd.append('is_primary', done === 0 && isPrimary ? 1 : 0);
            fd.append('image',      file);
            return fetch('/MantaMarket/sellers/products/ajax_edit.php', { method:'POST', body:fd })
                .then(r => r.json())
                .then(res => {
        ...
        fileArr.reduce((chain, file) => chain.then(() => uploadOne(file)), Promise.resolve())
            .then(() => {
                ...
                setTimeout(() => { if (prog) prog.innerHTML = ''; }, 4000);
            });
    }
    ...
    function vpAppendImageCard(productId, img, productArrIdx) {
        ...
        card.innerHTML = `
        <div style="aspect-ratio:1;overflow:hidden;background:#fafafa;display:flex;align-items:center;justify-content:center">
            <img src="${img.image_url}" alt="" style="width:100%;height:100%;object-fit:contain"
                 onerror="this.parentElement.innerHTML='<span style=color:#ddd;font-size:11px>No img</span>'">
        </div>
        <div style="padding:5px 7px;font-size:11px;color:#999;flex:1;line-height:1.4">
            ${img.variant_color ? `<div style="color:#7c3aed">● ${img.variant_color}</div>` : ''}
        ...
        <div style="padding:0 6px 7px;display:flex;gap:4px">
            ...
            <button title="Đặt làm ảnh chính"
                style="flex:1;padding:4px 0;border:1px solid #e0e0e0;border-radius:5px;font-size:11px;cursor:pointer;background:#fff;color:#7c3aed"
                onmouseover="this.style.background='#faf5ff'" onmouseout="this.style.background='#fff'">
                <i class="fas fa-star"></i>
            </button>` : '<div style="flex:1"></div>'}
            <button title="Xóa ảnh"
                style="flex:1;padding:4px 0;border:1px solid #fdd;border-radius:5px;font-size:11px;cursor:pointer;background:#fff;color:#ee4d2d"
                onmouseover="this.style.background='#fff0f0'" onmouseout="this.style.background='#fff'">
                <i class="fas fa-trash"></i>
            </button>
        </div>`;
