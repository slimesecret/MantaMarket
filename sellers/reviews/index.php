<?php
/**
 * sellers/reviews/index.php
 * Chỉ load review từ các sản phẩm thuộc seller đang đăng nhập.
 */
...
// ── Lấy sellerId từ session (dùng đúng key 'userId' như sellers/index.php) ──
// $sellerId đã được set bởi sellers/index.php trước khi include file này,
// nhưng để file có thể chạy độc lập thì fallback về session nếu chưa có.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'reply' && $id) {
    ...
    if ($reply !== '') {
        // Chỉ update review thuộc sản phẩm của seller này
        $db->query(
            "UPDATE reviews r
             JOIN   products p ON p.id = r.product_id AND p.seller_id = ?
             SET    r.reply = ?, r.replied_at = NOW(), r.is_approved = 1
             WHERE  r.id = ?",
            [$sellerId, $reply, $id]
        );
    }
    ...
    header("Location: /MantaMarket/sellers/index.php#reviews");
    exit();
}

// ── XÓA 1 REVIEW ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete' && $id) {
    // Chỉ xóa review thuộc sản phẩm của seller này
    $db->query(
...
// ── LẤY DỮ LIỆU TỪ DATABASE (chỉ reviews của sản phẩm thuộc seller) ──
try {
    $reviewRows = $db->query(
        "SELECT r.*,
...
// ── AJAX endpoint: trả JSON nếu request từ fetch() ──
...
let fbCurrentSort   = 'newest_review';
...
function fbSetTab(el, filter) {
    document.querySelectorAll('.fb-tab-group > .fb-tab').forEach(t => t.classList.remove('active'));
    el.classList.add('active');
...
function fbBulkDelete() {
    ...
    fetch('/MantaMarket/sellers/index.php?page=reviews', {
        method: 'POST',
        headers: {
...
function submitFbReply() {
    ...
    fetch('/MantaMarket/sellers/index.php?page=reviews', {
        ...
        body: new URLSearchParams({ action: 'reply', id, reply: text })
    })
    .then(res => {
